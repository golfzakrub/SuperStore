...
class Ui_Filter_Date_Window(object):
    ...
    def filter_date(self):
        ...
        if self.textEdit.toPlainText() == "" or  self.comboBox.currentText() == "":
            return
        for i in range(self.listWidget.count()):
            if "-" in self.listWidget.item(i).text():
                split_date = self.listWidget.item(i).text().split("-")
            if "/" in self.listWidget.item(i).text():
                split_date = self.listWidget.item(i).text().split("/")
            ...
            if self.comboBox2.currentText() == "Year":
                x = int(split_date[2])
            y = int(self.textEdit.toPlainText())
            if  op[self.comboBox.currentText()](x,y):
                self.listWidget.item(i).setCheckState(QtCore.Qt.Checked)
            else:
                self.listWidget.item(i).setCheckState(~(QtCore.Qt.Checked))
        ...
        if fig == "bar":
            ...
            if ((count_Column ==1 and len(row_index) !=0)  or (count_Row ==1 and len(col_index) !=0)):
                ...
                if len(col_index) == 2 and row_index[0] in Measurement:
                    ...
                    if filter_str == "":
                        alt.data_transformers.disable_max_rows()
                        chart = (alt.Chart(self.all_data[data].groupby(by=[col_index[0],col_index[1]],as_index=False).sum()) #replace chart_list array(1035)
                        .mark_bar()
                ...
                if len(row_index) == 2 and col_index[0] in Measurement:
                    ...
                    if filter_str == "":
                        alt.data_transformers.disable_max_rows()
                        chart = (alt.Chart(self.all_data[data].groupby(by=[row_index[0],row_index[1]],as_index=False).first()) #replace chart_list array(1035)
                        .mark_bar()
                        .encode(x= alt.X(col_index[0]),
